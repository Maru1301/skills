...
5. Follow a reliably established repository message style. Otherwise use an imperative Conventional Commit subject, `type(scope): summary` or `type: summary`, with a type that reflects the diff. Add a body only for useful context. Do not invent scopes, issue IDs, breaking changes, or motivations. Create the commit and verify its hash, subject, committed scope, and remaining status.
